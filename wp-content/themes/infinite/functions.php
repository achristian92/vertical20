<?php
	...
	*/
	...
	$sidebar_heading_tag = infinite_get_option('general', 'sidebar-heading-tag', 'h3');
	$sidebar_atts = array(
		'before_widget' => '<div id="%1$s" class="widget %2$s infinite-widget">',
		'after_widget'  => '</div>',
		'before_title'  => '<' . $sidebar_heading_tag . ' class="infinite-widget-title">',
		'after_title'   => '</' . $sidebar_heading_tag . '><span class="clear"></span>' );

	if( infinite_get_option('general', 'sidebar-title-divider', 'enable') == 'enable' ){
		$sidebar_atts['before_title'] = '<' . $sidebar_heading_tag . ' class="infinite-widget-title"><span class="infinite-widget-head-text">';
		$sidebar_atts['after_title'] = '</span><span class="infinite-widget-head-divider"></span></' . $sidebar_heading_tag . '><span class="clear"></span>';
	}
					...
					new gdlr_core_page_option(array(
						...
						'options' => array(
							'layout' => array(
								...
								'options' => array(
									'custom-header' => array(
										'title' => esc_html__('Select Custom Header', 'infinite'),
										'type' => 'combobox',
										'single' => 'gdlr_core_custom_header_id',
										'options' => array('' => esc_html__('Default', 'infinite')) + gdlr_core_get_post_list('gdlr_core_header')
									),
									...
									'enable-breadcrumbs' => array(
										'title' => esc_html__('Enable Breadcrumbs', 'infinite'),
										'type' => 'checkbox',
										'default' => 'disable',
										'condition' => array( 'enable-header-area' => 'enable', 'enable-page-title' => 'enable' )
									),
	...
	add_filter('gdlr_core_portfolio_options', 'infinite_gdlr_core_portfolio_options');
	if( !function_exists('infinite_gdlr_core_portfolio_options') ){
		function infinite_gdlr_core_portfolio_options($options){
